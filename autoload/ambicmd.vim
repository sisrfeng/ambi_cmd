if !exists('g:ambicmd#build_rule')  | let g:ambicmd#build_rule = 'ambicmd#default_rule'  | endif

if !exists('g:ambicmd#show_completion_menu')  | let g:ambicmd#show_completion_menu = 0  | endif
...
function! s:PATTERN_NON_ESCAPE_CHAR(char, ...) abort
    let force_escape = exists('a:1')
                    \ ? a:1
                    \ : 1

    return '\v%(%(\_^|[^\\])%(\\\\)*)@<=' .  (force_escape
                                    \ ? s:escape_for_very_magic(a:char)
                                    \ : a:char)
endfunction
